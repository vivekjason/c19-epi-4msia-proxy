...
#develop the data 
trans_region <- transmission %>% select(state, date, rollcases_7d, population_new) %>%
 mutate(region = ifelse(state == 'Malaysia', "Malaysia",
  ...
  mutate(rollcases_7d=sum(rollcases_7d),
         population_new=sum(population_new)) %>%
  ungroup() %>% unique() %>%
  mutate(date=as.Date(date),
         ir_7day=(rollcases_7d/population_new)*100000) %>%
  select(date, region, ir_7day)
...
# Plot
ir_curve_state <- plot_ly() %>%
  add_lines(data=transmission, x=~date, y = ~ir_7day, name=~state, color=~state,
  ...
  layout(titlefont=list(size=12),
         ...
         yaxis = list(side="left", tickfont= list(color = 'grey9', size=8), color='grey9', 
                      range=c(0,max(transmission$ir_7day, na.rm=T)*1.1),
...
#save the plot
saveRDS(ir_curve_state, "plots/ir_curve_state.rds")

#######################################################################################################
ir_curve_state_log10 <- plot_ly() %>%
  add_lines(data=transmission, x=~date, y = ~ir_7day, name=~state, color=~state,
            colors=c("#0000FF", "#63B8FF", "#006400", "#008B45", "#54FF9F", "#C71585", "#EEC900", "#FF8C00", "#68228B", "#FFAEB9", "#CD2626", "#008B8B", "#ADD8E6",
                     "#F08080", "#696969", "#2B2B2B", "#FF1493"))%>%
  layout(titlefont=list(size=12),
...
#save the plot
saveRDS(ir_curve_state_log10, "plots/ir_curve_statelog10.rds")

#######################################################################################################
ir_curve_region <- plot_ly() %>%
  add_lines(data=trans_region, x=~date, y = ~ir_7day, name=~region, color=~region,
  ...
  layout(titlefont=list(size=12),
         ...
         yaxis = list(side="left", tickfont= list(color = 'grey9', size=8), color='grey9', 
                      title="7-day incidence rate",
                      range=c(0,max(trans_region$ir_7day, na.rm=T)*1.1)),
         legend = list(x = 1.05, y = 0.9)) %>%
  config(modeBarButtonsToRemove = c("zoomIn2d", "zoomOut2d", "autoScale2d", "resetScale2d", "hoverClosestCartesian", "hoverCompareCartesian","lasso2d", "select2d", "pan2d","zoom2d")) %>%
  layout(legend = l,
         margin=m)
...
#save the plot
saveRDS(ir_curve_region, "plots/ir_curve_region.rds")

#######################################################################################################
ir_curve_region_log10 <- plot_ly() %>%
  add_lines(data=trans_region, x=~date, y = ~ir_7day, name=~region, color=~region,
            colors=c("#0000FF", "#CD2626", "#008B45", "#63B8FF", "#2B2B2B", "#FFAEB9", "#EEC900"))%>%
  layout(titlefont=list(size=12),
         xaxis = list(title="Date",
...
#save the plot
saveRDS(ir_curve_region_log10, "plots/ir_curve_regionlog10.rds")
